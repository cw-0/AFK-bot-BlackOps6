...
void TUI::run() {
  while (true) {
    ...
    case 2:
      settingsMenu();
      continue; // this continue stops it from running ShellExecute when going
                // from Settings -> Main menu
...
void TUI::printTitle() {
 ...
 | (_) |  / ____ \| |    | . \  | |_) | (_) | |_
  ...
  std::cout << YELLOW << "[ Left Alt ] - Pause Bot" << RESET << "|" << GREEN
            << " [ Right Alt ] - Resume Bot " << RESET << "|" << RED
            << " [ Right Ctrl ] - Exit\n"
            << RESET;
  std::cout << "\n\n";
}
...
void TUI::startBot(int waitTime) {
  ...
  for (int i = waitTime; i > 0; i--) {
    if (IS_PAUSED) {
      std::cout << "Pausing After Countdown.\n";
    }
...
void TUI::settingsMenu() {
  ...
  while (true) {
    ...
    std::cout << "2. Log To File: "
              << (LOG_FILE_ENABLED ? GREEN + "ENABLED" : RED + "DISABLED")
              << RESET << "\n";
    std::cout << "3. Toggle Log File Verbosity: "
              << (SEND_VERBOSE ? GREEN + "ON" : RED + "OFF") << RESET << "\n";
